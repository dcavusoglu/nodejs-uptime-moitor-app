...
handlers._users.get = function(data, callback){
  ...
  if (phone){
    ...
    // Verify taht the given token is valid for the phone number
  ...
  if(protocol && url && method && successCodes && timeoutSeconds){
    ...
    _data.read('tokens', token, function(err,tokenData){
      if(!err && tokenData){
        ...
        // Look up the ıser data
        _data.read('users', userPhone, function(err, userData){
          if(!err && userData) {
            const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];

            // Verify that the user has less than the num of max-checks-per-user
            if(userChecks.length < config.maxChecks){
              // Create a random id for the check
              const checkId = helpers.createRandomString(20);

              // Create the ckheck objecti and include the user's phone
              const checkObject = {
                'id' : checkId,
                'userPhone' : userPhone,
                'url' : url,
              ...
              callback(400, {'Error' : 'The user already has the mx num of checks ('+config.maxChecks+')'});
            }
          }
        })
      } else {
  ...
  if (id){
    ...
    _data.read('checks', id, function(err, checkData){
      if(!err && checkData){
        ...
        // Verify taht the given token is valid and belongs to the user who created the check
...
handlers._checks.put = function(data, callback){
  ...
  if (id) {
    // check if one or two optional fields are sent
    if(protocol || method || url || successCodes || timeoutSeconds){
      ...
      _data.read('checks', id, function(err, checkData){
        ...
        handlers._tokens.verifyToken(token, checkData.userPhone, function(tokenIsValid){
          if(tokenIsValid){
            // Update the check wheere necessary
            if(protocol){
